...
# ====== 下载 OpenClash（使用固定版本，避免 API 限流）======
echo "📥 下载 OpenClash 固定版本..."
OPENCLASH_VERSION="0.46.038"
...
if ! wget -q --timeout=60 --tries=3 -O luci-app-openclash.ipk "$OPENCLASH_URL"; then
    echo "❌ OpenClash 下载失败，请检查网络或版本是否存在"
    exit 1
fi
...
if ! wget -q --timeout=60 --tries=3 -O- "$META_URL" | tar -xz -C clash-core clash.meta; then
...
# GeoIP / GeoSite
wget -q --timeout=30 --tries=2 https://github.com/Loyalsoldier/v2ray-rules-dat/releases/latest/download/geoip.dat -O GeoIP.dat
wget -q --timeout=30 --tries=2 https://github.com/Loyalsoldier/v2ray-rules-dat/releases/latest/download/geosite.dat -O GeoSite.dat
